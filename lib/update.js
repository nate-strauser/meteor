...
updater.get_manifest(function (manifest) {
  ...
  var run_post_upgrade = function () {
    ...
    var postup_path = path.join(files.getCurrentEngineDir(), 'meteor', 'post-upgrade.js');
  ...
  if ('deb' === package_stamp) {
    ...
    download_callback = function (tar_path) {
      ...
      try {
        ...
        files.extractTarGz(fs.readFileSync(tar_path),
                           tmp_dir);
      } catch (e) {
        console.log(e);
  ...
  var req = https.request(download_parsed, function(res) {
    ...
    res.on('end', function () {
      ...
      // create a Fiber so that code in download_callback can use
      // sync calls having Futures.
      Fiber(function () {
